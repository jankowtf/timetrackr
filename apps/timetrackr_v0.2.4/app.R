...
server <- function(input, output, session) {
  ...
  output$dt_issues <- DT::renderDataTable({
    ...
    options = list(
      ...
      scrollX = TRUE,
      # scrollY = "500px",
      scrollY = sprintf("%spx", as.numeric(GLOBAL$ui$main$issues$left) * 0.67),
      scrollCollapse = TRUE,
  ...
  output$dt_times <- DT::renderDataTable({
    ...
    options = list(
    dom = "ltipr",
    scrollY = sprintf("%spx", as.numeric(GLOBAL$ui$main$times$left) * 0.55),
    scrollCollapse = TRUE,
    lengthMenu = list(
      c(5, 10, 15, 20, -1),
      c(5, 10, 15, 20, "All")
  ...
  output$dt_projects <- DT::renderDataTable({
    ...
    options = list(
      ...
      # autoWidth = TRUE,
      scrollX = TRUE,
      scrollY = sprintf("%spx", as.numeric(GLOBAL$ui$main$projects$left) * 0.55),
      scrollCollapse = TRUE,
